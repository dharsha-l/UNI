...
const InstitutionDetailPage: React.FC = () => {
  ...
  const quickActions = [
    { label: isInstitution ? 'View Inspection Findings' : 'View AI Findings', icon: AlertTriangle, path: '/findings' },
    { label: 'Inspection History', icon: History, path: '/history' },
    { label: 'Evidence Library', icon: FileStack, path: '/evidence' },
  ];

  if (!isInstitution) {
    quickActions.push({ label: 'View Reports', icon: TrendingUp, path: '/reports' });
  }
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="card p-6 mb-6">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-5">
            <div className={`w-16 h-16 ${isInstitution ? 'bg-emerald-600' : 'bg-blue-600'} rounded-2xl flex items-center justify-center`}>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                ...
                <span className={`badge ${isInstitution ? 'badge-low' : 'badge-blue'}`}>{institution.type}</span>
                <span className={`badge ${institution.accreditation_status.includes('Under') ? (isInstitution ? 'badge-low' : 'badge-blue') : 'badge-low'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="card overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={() => navigate('/history')} className={`text-sm font-medium flex items-center gap-1 ${isInstitution ? 'text-emerald-600 hover:text-emerald-800' : 'text-blue-600 hover:text-blue-800'}`}>
            ...
            <table className="data-table">
              ...
              <tbody>
                {history.length === 0 ? (
                  <tr><td colSpan={isInstitution ? 3 : 4} className="text-center text-slate-400 py-8">No inspections yet</td></tr>
                ) : history.map(h => (
                  <tr key={h.id} className="cursor-pointer" onClick={() => navigate(`/inspections/${h.id}`)}>
                    <td className={`font-mono text-sm ${isInstitution ? 'text-emerald-600' : 'text-blue-600'}`}>{h.inspection_id}</td>
                    ...
                    <td><span className={`badge ${h.status === 'Completed' ? 'badge-low' : h.status === 'In Progress' ? (isInstitution ? 'badge-low' : 'badge-blue') : 'badge-gray'}`}>{h.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="space-y-5">
          {/* Current risk */}
          {!isInstitution && latestInspection && (
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2">
              {quickActions.map(({ label, icon: Icon, path }) => (
                <button key={label} onClick={() => navigate(path)} className={`w-full flex items-center gap-3 text-sm text-slate-600 p-2.5 rounded-lg transition-colors ${isInstitution ? 'hover:text-emerald-600 hover:bg-emerald-50' : 'hover:text-blue-600 hover:bg-blue-50'}`}>
